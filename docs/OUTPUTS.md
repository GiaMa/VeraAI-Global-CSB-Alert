...
| Community Engagement | `data/processed/community_engagement_classified.csv` | 208 | 934 KB | Engagement metrics by community |
| Alert Links | `data/alerts/veraai_alerts_links.csv` | 14,244 | 24 MB | Coordinated link alerts |
...
**Purpose**: Comprehensive engagement metrics aggregated by community for analysis.

**Provenance**: Aggregated from post-level engagement data across all detected coordinated content.
...
**Purpose**: Comprehensive log of all coordinated link alerts generated by the system.

**Provenance**: Appended during each monitoring cycle when coordinated links are detected.
...
2. **Full dataset**: Available via [external repository link]
3. **On request**: Contact maintainers for complete data
